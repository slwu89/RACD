...
# -------------------------------------------------------------------------------- #
#   make parameters for the mosquito
# -------------------------------------------------------------------------------- #

parameters_mosquito <- function(species=1){

  pars <- c(   ## Mosquito life cycle parameters:
  ...
  if(species==1){
    cat("making An. gambiae parameters\n")
    ...
    cat("making An. arabiensis parameters\n")
    ...
    cat("making An. funestus parameters\n")
    ...
    cat("species should be an integer in 1,2,3\n")
  }

  pars["delta"] <- 1/(pars[["tau1"]]+pars[["tau2"]]) # Inverse of gonotrophic cycle without ITNs/IRS
  pars["eggOV"] <- pars[["beta"]]*(exp(pars[["muV"]]/pars[["delta"]])-1)/pars[["muV"]] # Number of eggs per oviposition per mosquito
...
# -------------------------------------------------------------------------------- #
#   make parameters for the humans
# -------------------------------------------------------------------------------- #

parameters_humans <- function(){
